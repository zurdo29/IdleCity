...
const UI = {
    ...
    updateResourceDisplays() {
        ...
        if (this.elements.coinsCount) {
            this.animateNumberChange(this.elements.coinsCount, GameState.resources.coins);
        }
        
        if (this.elements.populationCount) {
            this.animateNumberChange(this.elements.populationCount, GameState.resources.population);
        }
        
        if (this.elements.happinessCount) {
            this.animateNumberChange(this.elements.happinessCount, Math.floor(GameState.resources.happiness));
        }
    ...
    generateAchievementsUI() {
        if (!this.elements.achievementsGrid || typeof Achievements === 'undefined') return;
    ...
    generateBuildingsUI() {
        if (!this.elements.buildingsGrid) return;
    ...
    createBuildingCard(buildingType, config) {
        ...
        card.innerHTML = `
            ...
            <div class="space-y-2 text-sm">
                ...
                <button id="buy${buildingType.charAt(0).toUpperCase() + buildingType.slice(1)}Btn" 
                        class="btn-enhanced w-full bg-game-primary hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white py-2 px-3 rounded-lg font-semibold text-sm">
                    Buy ${config.name}
                </button>
            </div>
        `;
    ...
    createUpgradeCard(upgradeType, config) {
        ...
        card.innerHTML = `
            ...
            <div class="space-y-2 text-sm">
                ...
                <button id="buy${upgradeType.charAt(0).toUpperCase() + upgradeType.slice(1)}Btn" 
                        class="btn-enhanced w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white py-2 px-3 rounded-lg font-semibold text-sm">
                    Upgrade
                </button>
            </div>
        `;
